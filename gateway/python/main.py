...
client.publish(topic="iot-2/evt/hello/fmt/json", msg='{"value":"hello","type":"GW","msgid":0}')
...
try:
    ...
    #        print(msg)
        ...
        if (len(recv_pkg) > 160):
            print("<<<<<<<<<<<<< Received FROM PYCOM message, length = %d" % len(recv_pkg))
            ...
            client.publish(topic="iot-2/evt/pycom/fmt/json", msg=c )
            if device_id == 10:
               print("publish node msg to cloud ")
               client.publish(topic="iot-2/evt/pycom/fmt/json", msg=msg )
               pycom.rgbled(colorgreen)  # green
               time.sleep(1)
            else:
                print("Error -- Only Device 10 - Wrong device id =" +str(device_id))
                pycom.rgbled(colorred)  # green
                time.sleep(1)
